...
const SECTIONS = [
  ...
  { key: 'education', name: 'Education', href: '/education', icon: GraduationCap },
  { key: 'experience', name: 'Experience', href: '/experience', icon: Briefcase },
  { key: 'projects', name: 'Projects', href: '/projects', icon: Code2 },
  { key: 'skills', name: 'Skills', href: '/skills', icon: Wrench },
  { key: 'resumes', name: 'Resumes', href: '/resumes', icon: FileStack },
] as const
    ...
    <div className="flex h-full flex-col overflow-hidden bg-s1">
      ...
      <div className="flex flex-1 flex-col overflow-y-auto p-4">
        <section className="mb-6">
          ...
          >
            {SECTIONS.map((item) => {
              const complete = item.key === 'resumes' ? true : isComplete(item.key)
              return (
                <Link
                  key={item.name}
                  href={item.href}
                ...
                >
                  ...
                  <span
                    ...
                    style={{
                      backgroundColor: complete ? 'var(--green)' : 'var(--muted2)',
                    }}
                    aria-hidden
                  />
                  {complete && (
          ...
          <div className="relative">
            ...
            >
              {completeness?.is_complete
                ? 'Your profile is complete. Generate a tailored resume for any job.'
                : 'Complete your profile sections to unlock AI-tailored resume generation.'}
            </p>
          </div>
        </section>
